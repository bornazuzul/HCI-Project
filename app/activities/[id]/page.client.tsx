...
export default function ActivityPageClient({
  ...
  // Format date on mount and when activity changes
  useEffect(() => {
    const activityDate = new Date(activity.date);
    const formatted = activityDate.toLocaleDateString("en-US", {
      weekday: "long",
      year: "numeric",
      month: "long",
      day: "numeric",
    ...
    setIsPast(activityDateTime < today);
  }, [activity.date]);

  // Function to refresh activity data from server
  ...
  const handleApplicationChange = async (newApplicantCount: number) => {
    // Update local state immediately for instant feedback
    setActivity((prev) => ({
      ...prev,
      currentApplicants: newApplicantCount,
    }));
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <ApplicantsList
                ...
                key={`applicants-${activity.currentApplicants}`}
              />
            </div>
              ...
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-5">
                <div className="text-center mb-5">
                  ...
                  <h3 className="text-lg font-bold text-gray-900 mb-2">
                    {isPast ? "Event Completed" : "Ready to Volunteer?"}
                  </h3>
                  <p className="text-gray-600 text-sm mb-4">
                    {isPast ? (
                      "This activity has already taken place. Thank you to all volunteers who participated!"
                ...
                {isPast ? (
                  <button
                    disabled
                    className="w-full py-3 px-4 bg-gray-300 text-gray-500 font-semibold rounded-lg cursor-not-allowed transition-colors flex items-center justify-center gap-2"
                  >
                    <svg
                      className="w-5 h-5"
                      fill="none"
